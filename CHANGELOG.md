...
- Backends no longer skip descriptor models for their operations. As a consequence, `execute-generators` can now
  correctly generate languages, and `modelcheck` will check language descriptors when checking languages. Descriptor
  models can still be filtered out by excluding `.*@descriptor`.
...
- `execute`: if the method being executed returns an integer value, use the value as the exit code of the backend.

### Dependencies

- Upgrade to project-loader 5.0.1.

## 1.24.3

- Upgrade to project-loader 5.0.0
