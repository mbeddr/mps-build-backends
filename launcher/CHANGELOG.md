...
- Configure IntelliJ Platform logging to run synchronously, avoiding writes to its asynchronous log queue during
  shutdown after the queue has closed.

## 2.9.1

### Fixed

- Pass `idea.config.path` and `idea.system.path` system properties to MPS through a `CommandLineArgumentProvider` to
  stop them from affecting Gradle build cache keys.

## 2.9.0

### Added

- Specify JDK 25 for MPS 2026.1 and above.
